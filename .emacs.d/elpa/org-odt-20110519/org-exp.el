      ...
      (setq rtn
		...
		(setq rtn (org-export-number-lines rtn 1 1 num cont rpllbl fmt)))
	     ...
	     ((eq org-export-current-backend 'odt)
	      (concat "\n#+BEGIN_ODT\n"
		      (org-add-props
			  (org-odt-format-example
			   (org-export-number-lines
			    (org-html-protect-lines rtn) 0 0 num cont rpllbl fmt))
...
(defun org-export-number-lines (text &optional skip1 skip2 number cont
  ...
  (with-temp-buffer
    ...
    (let* ((last (org-current-line))
	   ...
	   (fm
	    (cond
	     ((member org-export-current-backend '(html odt))
	      (org-parse-format 'FONTIFY fmt "linenr"))
	...
	(when (looking-at lbl-re)
	  ...
	  (when (member org-export-current-backend '(html odt))
	    (beginning-of-line 1)
	    (re-search-forward "^\\(.*\\)$" nil t 1)
	    (replace-match
	     (let ((id (concat "coderef-" ref)))
	       (org-parse-format
		'ANCHOR (match-string 1) id "coderef-off")) t t))))
