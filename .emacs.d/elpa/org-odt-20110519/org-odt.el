...
;; Refer comments in org-newhtml.el
(defvar org-basehtml-special-paragraph-styles)
...
(defconst org-odt-data-dir
  (let ((dir1 (expand-file-name "../contrib/odt" org-odt-lib-dir)) ; git
...
(eval-after-load 'rng-loc
  '(add-to-list 'rng-schema-locating-files
		(expand-file-name "schemas.xml" org-odt-data-dir)))
...
headlines.  The default is 3.  Lower levels will become bulleted lists."
  (interactive "P")
  (org-export-as-and-open "odt" arg))
...
No file is created.  The prefix ARG is passed through to `org-export-as-odt'."
  (interactive "P")
  (org-export-as-to-buffer "odt" arg))
...
in a window.  A non-interactive call will only return the buffer."
  (interactive "r\nP")
  (org-export-region-as "odt" beg end body-only buffer))
...
PUB-DIR is set, use this as the publishing directory."
  (interactive "P")
  (org-export-as "odt" arg hidden ext-plist to-buffer body-only pub-dir))
...
;; Following variable is let bound when `org-do-export' is in
...
(defun org-odt-begin-document-body (opt-plist)
  ...
  ;; insert toc
  (when org-parse-table-of-contents
    (insert "\n" org-parse-table-of-contents "\n")))

(defvar org-parse-body-only)		; let bound during org-do-export
(defvar org-parse-to-buffer)		; let bound during org-do-export
(defun org-odt-end-document-body (opt-plist)
  (unless org-parse-body-only
    (org-html-insert-tag "</office:text>")
    (org-html-insert-tag "</office:body>")))
...
(defun org-odt-begin-document-content (opt-plist)
  ...
  (insert-file-contents
   (or org-export-odt-automatic-styles-file
       (expand-file-name "OrgOdtAutomaticStyles.xml" org-odt-data-dir)))
   (goto-char (point-max)))

(defun org-odt-end-document-content ()
  (org-html-insert-tag "</office:document-content>"))
  ...
  (let* ((style-name (org-odt-get-style-name-for-entity 'list ltype))
    ...
    (case ltype
      ((ordered unordered description)
       (org-parse-end-paragraph)
       (org-html-insert-tag "<text:list%s>" extra))
  ...
  (case ltype
    (ordered
     ...
     (let* ((counter arg) (extra ""))
       (org-html-insert-tag "<text:list-item>")
       (org-parse-begin-paragraph)))
    (unordered
     (let* ((id arg) (extra ""))
       (org-html-insert-tag "<text:list-item>")
	...
	(org-html-format-tags
	 ...
	 (org-odt-format-stylized-paragraph 'definition-term term)))
       ...
       (org-parse-begin 'LIST 'description)
       (org-parse-begin 'LIST-ITEM 'unordered)))
...
(defun org-odt-end-list-item (ltype)
  ...
  (case ltype
    ...
    (description
     (org-close-li)
     (org-parse-end 'LIST 'description)
     (org-close-li))
    (t (error "Unknown list type"))))
...
(defun org-odt-end-table ()
  ...
  (loop for level from 0 below org-table-ncols
	do (insert (org-html-format-tags "<table:table-column/>"  "")))
  ...
  (goto-char (point-max))
  (org-html-insert-tag "</table:table>"))
...
(defun org-odt-get-style-name-for-table-cell (r c)
  (concat
   ...
   (cond
    ((= r 0) "T")
    ((eq (cdr (assoc r org-table-rowgroup-info))  :start) "T")
    (t ""))
   (when (= r org-table-rownum) "B")
   (cond
    ((= c 0) "")
...
(defun org-odt-get-paragraph-style-for-table-cell (r c)
  (capitalize (aref org-table-colalign-vector c)))

(defun org-odt-format-table-cell (data r c)
  (if (not org-table-is-styled)
      (org-html-format-tags
       '("<table:table-cell>" . "</table:table-cell>")
       (org-odt-format-stylized-paragraph
	(cond
	 (org-table-current-rowgroup-is-header "OrgTableHeading")
...
(defun org-odt-format-toc-item (toc-entry level org-last-level)
  (let ((style (format "Contents_20_%d"
		       (+ level (or (org-parse-get 'TOPLEVEL-HLEVEL) 1) -1))))
    (insert "\n" (org-odt-format-stylized-paragraph style toc-entry) "\n")))

;; Following variable is let bound during 'ORG-LINK callback. See
;; org-html.el
(defvar org-parse-link-description-is-image nil)
...
(defun org-odt-format-spaces (n)
  (org-html-format-tags "<text:s text:c=\"%d\"/>" "" n))
...
(defun org-odt-format-line-break ()
  (org-html-format-tags "<text:line-break/>" ""))
...
(defun org-html-protect-lines (rtn)
  (mapconcat
   (lambda (line) (org-html-protect line))
...
MAY-INLINE-P allows inlining it as an image."
  ...
  (save-match-data
	...
	;; (setq thefile (org-export-html-format-href thefile))
	...
	(let ((org-odt-suppress-xref (string= type "coderef")))
	  (org-odt-format-link
	   (org-export-html-format-desc desc) thefile attr)))))))
...
(defun org-export-odt-format-image (src href
  ...
  (save-match-data
	...
	(setq caption (org-find-text-property-in-string 'org-caption src)
	      caption (and caption (org-export-html-format-desc caption))
...
(defun org-odt-init-outfile (filename)
  (let* ((outdir (make-temp-file org-export-odt-tmpdir-prefix t))
    ...
    ;; content file
    (with-current-buffer (find-file-noselect content-file) (erase-buffer))
...
(defun org-odt-save-as-outfile (target opt-plist)
  ...
  (with-current-buffer
      (find-file-noselect "styles.xml" (file-name-directory buffer-file-name))
  ...
  (let ((zipdir default-directory))
	...
	(or (zerop exitcode)
      ...
      (mapc (lambda (file)
	      (kill-buffer (find-file-noselect (expand-file-name file zipdir))))
	    org-export-odt-save-list)

      (delete-directory zipdir)))

  ;; FIXME: How to open the file without visiting
  (find-file target))
...
(defun org-odt-update-meta-file (opt-plist)
  (with-current-buffer
    ...
    (let ((date (or (plist-get opt-plist :effective-date) ""))
      ...
      (insert
       "\n"
       (org-html-format-tags '("<dc:creator>" . "</dc:creator>") author)
       (org-html-format-tags
...
(defun org-odt-update-manifest-file (media-type full-path)
  (with-current-buffer
      (find-file-noselect "META-INF/manifest.xml")
...
(defvar org-parse-latex-fragment-fallback) ; set by org-do-export
(defun org-export-odt-preprocess (parameters)
  ...
  (when (and org-current-export-file
	...
	(concat
	 ...
	 (format "Using %S instead."  org-parse-latex-fragment-fallback)))
	 org-parse-latex-fragment-fallback)
...
(defun org-odt-copy-styles-file (&optional styles-file)
  ...
  (setq styles-file (or styles-file
			...
			(expand-file-name "OrgOdtStyles.xml" org-odt-data-dir)
...
(defun org-odt-unit-test (&optional linger)
  ...
  (interactive "P")
  (with-current-buffer
      (find-file (expand-file-name "files/test.org" org-odt-data-dir))
    (unless linger
